"""
LCD Fonts

Bitmap font definitions for G13 LCD display.
"""

from dataclasses import dataclass


@dataclass
class Font:
    """Bitmap font definition."""

    char_width: int
    char_height: int
    glyphs: dict[int, list[int]]  # ASCII code -> column bytes

    def get_glyph(self, char: str) -> list[int] | None:
        """
        Get glyph data for character.

        Args:
            char: Character to look up

        Returns:
            List of column bytes, or None if not found
        """
        code = ord(char)
        if code in self.glyphs:
            return self.glyphs[code]
        # Return '?' for unknown characters
        return self.glyphs.get(63)


# 5x7 font - standard small font
# Each character is 5 columns, 7 rows high
# Each byte represents one column, bit 0 is top row
_FONT_5X7_GLYPHS = {
    32: [0x00, 0x00, 0x00, 0x00, 0x00],  # space
    33: [0x00, 0x00, 0x5F, 0x00, 0x00],  # !
    34: [0x00, 0x07, 0x00, 0x07, 0x00],  # "
    35: [0x14, 0x7F, 0x14, 0x7F, 0x14],  # #
    36: [0x24, 0x2A, 0x7F, 0x2A, 0x12],  # $
    37: [0x23, 0x13, 0x08, 0x64, 0x62],  # %
    38: [0x36, 0x49, 0x55, 0x22, 0x50],  # &
    39: [0x00, 0x05, 0x03, 0x00, 0x00],  # '
    40: [0x00, 0x1C, 0x22, 0x41, 0x00],  # (
    41: [0x00, 0x41, 0x22, 0x1C, 0x00],  # )
    42: [0x08, 0x2A, 0x1C, 0x2A, 0x08],  # *
    43: [0x08, 0x08, 0x3E, 0x08, 0x08],  # +
    44: [0x00, 0x50, 0x30, 0x00, 0x00],  # ,
    45: [0x08, 0x08, 0x08, 0x08, 0x08],  # -
    46: [0x00, 0x60, 0x60, 0x00, 0x00],  # .
    47: [0x20, 0x10, 0x08, 0x04, 0x02],  # /
    48: [0x3E, 0x51, 0x49, 0x45, 0x3E],  # 0
    49: [0x00, 0x42, 0x7F, 0x40, 0x00],  # 1
    50: [0x42, 0x61, 0x51, 0x49, 0x46],  # 2
    51: [0x21, 0x41, 0x45, 0x4B, 0x31],  # 3
    52: [0x18, 0x14, 0x12, 0x7F, 0x10],  # 4
    53: [0x27, 0x45, 0x45, 0x45, 0x39],  # 5
    54: [0x3C, 0x4A, 0x49, 0x49, 0x30],  # 6
    55: [0x01, 0x71, 0x09, 0x05, 0x03],  # 7
    56: [0x36, 0x49, 0x49, 0x49, 0x36],  # 8
    57: [0x06, 0x49, 0x49, 0x29, 0x1E],  # 9
    58: [0x00, 0x36, 0x36, 0x00, 0x00],  # :
    59: [0x00, 0x56, 0x36, 0x00, 0x00],  # ;
    60: [0x00, 0x08, 0x14, 0x22, 0x41],  # <
    61: [0x14, 0x14, 0x14, 0x14, 0x14],  # =
    62: [0x41, 0x22, 0x14, 0x08, 0x00],  # >
    63: [0x02, 0x01, 0x51, 0x09, 0x06],  # ?
    64: [0x32, 0x49, 0x79, 0x41, 0x3E],  # @
    65: [0x7E, 0x11, 0x11, 0x11, 0x7E],  # A
    66: [0x7F, 0x49, 0x49, 0x49, 0x36],  # B
    67: [0x3E, 0x41, 0x41, 0x41, 0x22],  # C
    68: [0x7F, 0x41, 0x41, 0x22, 0x1C],  # D
    69: [0x7F, 0x49, 0x49, 0x49, 0x41],  # E
    70: [0x7F, 0x09, 0x09, 0x01, 0x01],  # F
    71: [0x3E, 0x41, 0x41, 0x51, 0x32],  # G
    72: [0x7F, 0x08, 0x08, 0x08, 0x7F],  # H
    73: [0x00, 0x41, 0x7F, 0x41, 0x00],  # I
    74: [0x20, 0x40, 0x41, 0x3F, 0x01],  # J
    75: [0x7F, 0x08, 0x14, 0x22, 0x41],  # K
    76: [0x7F, 0x40, 0x40, 0x40, 0x40],  # L
    77: [0x7F, 0x02, 0x04, 0x02, 0x7F],  # M
    78: [0x7F, 0x04, 0x08, 0x10, 0x7F],  # N
    79: [0x3E, 0x41, 0x41, 0x41, 0x3E],  # O
    80: [0x7F, 0x09, 0x09, 0x09, 0x06],  # P
    81: [0x3E, 0x41, 0x51, 0x21, 0x5E],  # Q
    82: [0x7F, 0x09, 0x19, 0x29, 0x46],  # R
    83: [0x46, 0x49, 0x49, 0x49, 0x31],  # S
    84: [0x01, 0x01, 0x7F, 0x01, 0x01],  # T
    85: [0x3F, 0x40, 0x40, 0x40, 0x3F],  # U
    86: [0x1F, 0x20, 0x40, 0x20, 0x1F],  # V
    87: [0x7F, 0x20, 0x18, 0x20, 0x7F],  # W
    88: [0x63, 0x14, 0x08, 0x14, 0x63],  # X
    89: [0x03, 0x04, 0x78, 0x04, 0x03],  # Y
    90: [0x61, 0x51, 0x49, 0x45, 0x43],  # Z
    91: [0x00, 0x00, 0x7F, 0x41, 0x41],  # [
    92: [0x02, 0x04, 0x08, 0x10, 0x20],  # \
    93: [0x41, 0x41, 0x7F, 0x00, 0x00],  # ]
    94: [0x04, 0x02, 0x01, 0x02, 0x04],  # ^
    95: [0x40, 0x40, 0x40, 0x40, 0x40],  # _
    96: [0x00, 0x01, 0x02, 0x04, 0x00],  # `
    97: [0x20, 0x54, 0x54, 0x54, 0x78],  # a
    98: [0x7F, 0x48, 0x44, 0x44, 0x38],  # b
    99: [0x38, 0x44, 0x44, 0x44, 0x20],  # c
    100: [0x38, 0x44, 0x44, 0x48, 0x7F],  # d
    101: [0x38, 0x54, 0x54, 0x54, 0x18],  # e
    102: [0x08, 0x7E, 0x09, 0x01, 0x02],  # f
    103: [0x08, 0x14, 0x54, 0x54, 0x3C],  # g
    104: [0x7F, 0x08, 0x04, 0x04, 0x78],  # h
    105: [0x00, 0x44, 0x7D, 0x40, 0x00],  # i
    106: [0x20, 0x40, 0x44, 0x3D, 0x00],  # j
    107: [0x00, 0x7F, 0x10, 0x28, 0x44],  # k
    108: [0x00, 0x41, 0x7F, 0x40, 0x00],  # l
    109: [0x7C, 0x04, 0x18, 0x04, 0x78],  # m
    110: [0x7C, 0x08, 0x04, 0x04, 0x78],  # n
    111: [0x38, 0x44, 0x44, 0x44, 0x38],  # o
    112: [0x7C, 0x14, 0x14, 0x14, 0x08],  # p
    113: [0x08, 0x14, 0x14, 0x18, 0x7C],  # q
    114: [0x7C, 0x08, 0x04, 0x04, 0x08],  # r
    115: [0x48, 0x54, 0x54, 0x54, 0x20],  # s
    116: [0x04, 0x3F, 0x44, 0x40, 0x20],  # t
    117: [0x3C, 0x40, 0x40, 0x20, 0x7C],  # u
    118: [0x1C, 0x20, 0x40, 0x20, 0x1C],  # v
    119: [0x3C, 0x40, 0x30, 0x40, 0x3C],  # w
    120: [0x44, 0x28, 0x10, 0x28, 0x44],  # x
    121: [0x0C, 0x50, 0x50, 0x50, 0x3C],  # y
    122: [0x44, 0x64, 0x54, 0x4C, 0x44],  # z
    123: [0x00, 0x08, 0x36, 0x41, 0x00],  # {
    124: [0x00, 0x00, 0x7F, 0x00, 0x00],  # |
    125: [0x00, 0x41, 0x36, 0x08, 0x00],  # }
    126: [0x08, 0x08, 0x2A, 0x1C, 0x08],  # ~
}

FONT_5X7 = Font(char_width=5, char_height=7, glyphs=_FONT_5X7_GLYPHS)


# 4x6 font - tiny font for maximum text
_FONT_4X6_GLYPHS = {
    32: [0x00, 0x00, 0x00, 0x00],  # space
    33: [0x00, 0x2F, 0x00, 0x00],  # !
    45: [0x08, 0x08, 0x08, 0x00],  # -
    46: [0x00, 0x20, 0x00, 0x00],  # .
    48: [0x1E, 0x21, 0x21, 0x1E],  # 0
    49: [0x00, 0x22, 0x3F, 0x20],  # 1
    50: [0x32, 0x29, 0x29, 0x26],  # 2
    51: [0x12, 0x21, 0x25, 0x1A],  # 3
    52: [0x0C, 0x0A, 0x3F, 0x08],  # 4
    53: [0x17, 0x25, 0x25, 0x19],  # 5
    54: [0x1E, 0x25, 0x25, 0x18],  # 6
    55: [0x01, 0x39, 0x05, 0x03],  # 7
    56: [0x1A, 0x25, 0x25, 0x1A],  # 8
    57: [0x06, 0x29, 0x29, 0x1E],  # 9
    58: [0x00, 0x14, 0x00, 0x00],  # :
    65: [0x3E, 0x09, 0x09, 0x3E],  # A
    66: [0x3F, 0x25, 0x25, 0x1A],  # B
    67: [0x1E, 0x21, 0x21, 0x12],  # C
    68: [0x3F, 0x21, 0x21, 0x1E],  # D
    69: [0x3F, 0x25, 0x25, 0x21],  # E
    70: [0x3F, 0x05, 0x05, 0x01],  # F
    71: [0x1E, 0x21, 0x29, 0x3A],  # G
    72: [0x3F, 0x04, 0x04, 0x3F],  # H
    73: [0x21, 0x3F, 0x21, 0x00],  # I
    74: [0x10, 0x20, 0x21, 0x1F],  # J
    75: [0x3F, 0x04, 0x0A, 0x31],  # K
    76: [0x3F, 0x20, 0x20, 0x20],  # L
    77: [0x3F, 0x02, 0x02, 0x3F],  # M
    78: [0x3F, 0x02, 0x04, 0x3F],  # N
    79: [0x1E, 0x21, 0x21, 0x1E],  # O
    80: [0x3F, 0x09, 0x09, 0x06],  # P
    81: [0x1E, 0x21, 0x11, 0x2E],  # Q
    82: [0x3F, 0x09, 0x19, 0x26],  # R
    83: [0x22, 0x25, 0x29, 0x12],  # S
    84: [0x01, 0x3F, 0x01, 0x01],  # T
    85: [0x1F, 0x20, 0x20, 0x1F],  # U
    86: [0x0F, 0x10, 0x20, 0x1F],  # V
    87: [0x1F, 0x20, 0x10, 0x3F],  # W
    88: [0x31, 0x0A, 0x04, 0x31],  # X
    89: [0x03, 0x04, 0x38, 0x07],  # Y
    90: [0x31, 0x29, 0x25, 0x23],  # Z
}

FONT_4X6 = Font(char_width=4, char_height=6, glyphs=_FONT_4X6_GLYPHS)


# 8x8 font - larger font for headers
# Only uppercase + digits for simplicity
_FONT_8X8_GLYPHS = {
    32: [0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00],  # space
    48: [0x3C, 0x42, 0x42, 0x42, 0x42, 0x42, 0x3C, 0x00],  # 0
    49: [0x10, 0x18, 0x10, 0x10, 0x10, 0x10, 0x7C, 0x00],  # 1
    50: [0x3C, 0x42, 0x40, 0x20, 0x10, 0x08, 0x7E, 0x00],  # 2
    51: [0x3C, 0x42, 0x40, 0x1C, 0x40, 0x42, 0x3C, 0x00],  # 3
    52: [0x20, 0x30, 0x28, 0x24, 0x7E, 0x20, 0x20, 0x00],  # 4
    53: [0x7E, 0x02, 0x1E, 0x40, 0x40, 0x42, 0x3C, 0x00],  # 5
    54: [0x38, 0x04, 0x02, 0x3E, 0x42, 0x42, 0x3C, 0x00],  # 6
    55: [0x7E, 0x40, 0x20, 0x10, 0x08, 0x08, 0x08, 0x00],  # 7
    56: [0x3C, 0x42, 0x42, 0x3C, 0x42, 0x42, 0x3C, 0x00],  # 8
    57: [0x3C, 0x42, 0x42, 0x7C, 0x40, 0x20, 0x1C, 0x00],  # 9
    65: [0x18, 0x24, 0x42, 0x7E, 0x42, 0x42, 0x42, 0x00],  # A
    66: [0x3E, 0x42, 0x42, 0x3E, 0x42, 0x42, 0x3E, 0x00],  # B
    67: [0x3C, 0x42, 0x02, 0x02, 0x02, 0x42, 0x3C, 0x00],  # C
    68: [0x1E, 0x22, 0x42, 0x42, 0x42, 0x22, 0x1E, 0x00],  # D
    69: [0x7E, 0x02, 0x02, 0x3E, 0x02, 0x02, 0x7E, 0x00],  # E
    70: [0x7E, 0x02, 0x02, 0x3E, 0x02, 0x02, 0x02, 0x00],  # F
    71: [0x3C, 0x42, 0x02, 0x72, 0x42, 0x42, 0x3C, 0x00],  # G
    72: [0x42, 0x42, 0x42, 0x7E, 0x42, 0x42, 0x42, 0x00],  # H
    73: [0x3E, 0x08, 0x08, 0x08, 0x08, 0x08, 0x3E, 0x00],  # I
    74: [0x70, 0x40, 0x40, 0x40, 0x40, 0x42, 0x3C, 0x00],  # J
    75: [0x42, 0x22, 0x12, 0x0E, 0x12, 0x22, 0x42, 0x00],  # K
    76: [0x02, 0x02, 0x02, 0x02, 0x02, 0x02, 0x7E, 0x00],  # L
    77: [0x42, 0x66, 0x5A, 0x42, 0x42, 0x42, 0x42, 0x00],  # M
    78: [0x42, 0x46, 0x4A, 0x52, 0x62, 0x42, 0x42, 0x00],  # N
    79: [0x3C, 0x42, 0x42, 0x42, 0x42, 0x42, 0x3C, 0x00],  # O
    80: [0x3E, 0x42, 0x42, 0x3E, 0x02, 0x02, 0x02, 0x00],  # P
    81: [0x3C, 0x42, 0x42, 0x42, 0x52, 0x22, 0x5C, 0x00],  # Q
    82: [0x3E, 0x42, 0x42, 0x3E, 0x12, 0x22, 0x42, 0x00],  # R
    83: [0x3C, 0x42, 0x02, 0x3C, 0x40, 0x42, 0x3C, 0x00],  # S
    84: [0x7F, 0x08, 0x08, 0x08, 0x08, 0x08, 0x08, 0x00],  # T
    85: [0x42, 0x42, 0x42, 0x42, 0x42, 0x42, 0x3C, 0x00],  # U
    86: [0x42, 0x42, 0x42, 0x42, 0x24, 0x24, 0x18, 0x00],  # V
    87: [0x42, 0x42, 0x42, 0x42, 0x5A, 0x66, 0x42, 0x00],  # W
    88: [0x42, 0x42, 0x24, 0x18, 0x24, 0x42, 0x42, 0x00],  # X
    89: [0x41, 0x22, 0x14, 0x08, 0x08, 0x08, 0x08, 0x00],  # Y
    90: [0x7E, 0x40, 0x20, 0x18, 0x04, 0x02, 0x7E, 0x00],  # Z
}

FONT_8X8 = Font(char_width=8, char_height=8, glyphs=_FONT_8X8_GLYPHS)

# Convenience aliases
FONT_SMALL = FONT_4X6
FONT_MEDIUM = FONT_5X7
FONT_LARGE = FONT_8X8
