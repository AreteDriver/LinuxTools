...
def create_g13_layout():
    ...
    draw.rounded_rectangle([50, 50, WIDTH-50, HEIGHT-50], radius=30, fill=KEYBOARD_COLOR, outline=ACCENT_COLOR, width=3)

    # Draw LCD area
    draw.rectangle([LCD['x'], LCD['y'], LCD['x']+LCD['w'], LCD['y']+LCD['h']], fill=LCD_COLOR, outline=ACCENT_COLOR, width=2)

    # Draw M-keys
    for btn in M_KEYS:
        draw.rounded_rectangle(
            [btn['x'], btn['y'], btn['x']+btn['w'], btn['y']+btn['h']],
            radius=5, fill=M_BUTTON_COLOR, outline=(120, 120, 120), width=2
        )
        # Center text
    ...
    for row in [G_ROW1, G_ROW2, G_ROW3, G_ROW4]:
        for btn in row:
            draw.rounded_rectangle(
                [btn['x'], btn['y'], btn['x']+btn['w'], btn['y']+btn['h']],
                radius=5, fill=BUTTON_COLOR, outline=(140, 140, 140), width=2
            )
            # Center text
            bbox = draw.textbbox((0, 0), btn['name'], font=font_medium)
            text_width = bbox[2] - bbox[0]
    ...
    # Draw joystick area
    draw.ellipse(
        [JOYSTICK['x'], JOYSTICK['y'], JOYSTICK['x']+JOYSTICK['w'], JOYSTICK['y']+JOYSTICK['h']],
        fill=(60, 60, 60), outline=(140, 140, 140), width=3
    )
    ...
    draw.ellipse(
        ...
        fill=(80, 80, 80), outline=(150, 150, 150), width=2
    )
